...
class StepperCommand:
    ...
    def __del__(self):
        if self.traceDebug:
            print ("Reset motor")
        if self.pulseGpio:
            GPIO.output(self.pulseGpio, GPIO.LOW)
        if self.driverMinimalMicroSec:
            time.sleep(self.driverMinimalMicroSec / 1E6)
        if self.directionGpio:
            GPIO.output(self.directionGpio, GPIO.LOW)  
        if self.driverMinimalMicroSec:
            time.sleep(self.driverMinimalMicroSec / 1E6)
        if self.enableGpio:
            GPIO.output(self.enableGpio, GPIO.HIGH)
    ...
    def rotateAngle(self, angle):
        ...
        GPIO.output(self.directionGpio, GPIO.HIGH if microSteps >= 0 else GPIO.LOW)
        time.sleep(self.driverMinimalMicroSec / 1E6)
        
        # Rotate motor the right number of micro-steps
        for i in range(abs(microSteps)):
            GPIO.output(self.pulseGpio, GPIO.HIGH)
    ...
    def begin(self):
        ...
        GPIO.setup(self.pulseGpio, GPIO.OUT)
        ...
        if self.enableGpio:
            ...
            if self.traceDebug:
                print ("Enable motor")
            GPIO.output(self.enableGpio, GPIO.LOW)
            time.sleep(self.driverMinimalMicroSec / 1E6)
